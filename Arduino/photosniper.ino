...
# define PIN_RX 3
# define PIN_TX 2
# define PIN_IR 6 // IR LED Haxe 2
# define PIN_AF 7 // Pin for Autofocus-Optocoupler
# define PIN_SHUTTER 8 // Pin for Shutter-Optocoupler

// Camera Storage Delay
# define Camera_delay 250 //Time in ms for which the script will be delayed after one cyle in infinite mode. In this Time the camera will store the Photo on the SD Card. Increase Value if your camera skips some cycles.#define MIROR_UP false
...
# define CMD_BUFFER_LEN (byte) 4
# define CMD_SEQUENCEBUFFER_SIZE (byte) 128
...
char commndBuffer[CMD_SEQUENCEBUFFER_SIZE]; // an array to store the received data
...
void setup() { //initialize serial port for logs
...
void recvCommand() {
  static byte ndx = 0;
  byte cmdIx = 0;
  byte oneCmdBuffer[CMD_BUFFER_LEN] ;
  char rc; // maybe shorter -> bleSerial.readBytesUntil(CMD_END_CHAR, commndBuffer, sizeof(commndBuffer) / sizeof(char) );

  boolean inCmdMode = false;
  while (bleSerial.available() > 0) {
    rc = bleSerial.read(); // all commands are CMD_BUFFER_LEN byte sequences !! BEGIN & END TAGS are stripped

    if (!cam_busy) {
      oneCmdBuffer[cmdIx++] = rc;
      if (rc == CMD_BUFFER_LEN) // preprocess single cmd
      {
        cmdIx = 0; // check if we got a CMD_TAG

        if (inCmdMode) { // is not END_TAG
          ...
          if ((oneCmdBuffer[0] !=
              ...
              (oneCmdBuffer[2] !=
               CMD_END_TAG[2]) &&
              (oneCmdBuffer[3] !=
               CMD_END_TAG[3])) { // add cmd to buffer

            for (int i = 0; i < CMD_BUFFER_LEN; i++) {
              commndBuffer[ndx++] = oneCmdBuffer[i];
              if (ndx >= CMD_SEQUENCEBUFFER_SIZE) {
                ndx = CMD_SEQUENCEBUFFER_SIZE - 1;
              }
            }
          }
          else { // cmd sequence is ready - we goto processing
          ...
          inCmdMode = ((oneCmdBuffer[0] == CMD_START_TAG[0]) &&
                       (oneCmdBuffer[1] == CMD_START_TAG[1]) &&
                       (oneCmdBuffer[2] == CMD_START_TAG[2]) &&
                       (oneCmdBuffer[3] == CMD_START_TAG[3]));
        }
      }
    }
  }
...
void processCommandChain(char * cmdSequence, int bytesUsed) {
  ...
  int x = 0;
  while (x < bytesUsed) { // get a cmd with params

    byte * cmd = cmdSequence[x++];
    word * param1 = cmdSequence[x];
    x += 2;
    byte * param2 = cmdSequence[x++]; // check last cmd to be a loop ;-)
    if (processCommand(cmd, param1, param2)) { // we found a STOP cmdbreak;
    }
  }
}
boolean processCommand(byte cmd, word param1, byte param2) {
  ...
  switch (cmd) {
    ...
    case 0x02: // 'C'
      ...
      break;
    case 0x03: // 'D'
      for (int i = 0; i < param2; i++) // little delay
      {
        delay(param1);
      }
      break;
...
// B
void closeShutter() {
  if (shutterReleased) return; // close wire switch
  Serial.println("close Shutter");
  boolean MIROR_UP = false;
  if (MIROR_UP == true) { //Use if Mirror Up Setting is Activated //Trigger Mirror-Up is set
    ...
    delay(250); //Pause to reduce vibrations of the camera //Mirror is now UP, Start Trigger routine
  } //Set AF and Shutter Pin to high
  digitalWrite(PIN_AF, HIGH); //Activate Focus
  delay(40); //Give the Camera some time to react
  digitalWrite(PIN_SHUTTER, HIGH); // Activate Shutter

  shutterReleased = true; // close IR switch
  if (ir_enabled) {
...
// C
void openShutter() { // open wire switch
  Serial.println("open Shutter"); //Reset AF and Shutter Pin to LOW
  digitalWrite(PIN_SHUTTER, LOW); // Deactivate Shutter
  digitalWrite(PIN_AF, LOW); //Deactivate Focus //
...
void chooseIR(int cameraType) {
  ...
  if (ir_enabled) {
    if (cameraType != old_ir_id) { // delete old
      if (camera) {
        delete camera;
        camera = NULL;
      }
