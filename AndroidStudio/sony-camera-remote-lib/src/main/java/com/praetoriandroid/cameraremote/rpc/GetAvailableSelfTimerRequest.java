...
public class GetAvailableSelfTimerRequest extends BaseRequest<Void, GetAvailableSelfTimerResponse> {
    ...
    @Override
    public GetAvailableSelfTimerResponse parseResponse(Gson gson, String data) throws JsonSyntaxException {
    ...
    private Gson getGson() {
        return new GsonBuilder().registerTypeAdapter(CustomizableEntity.class, new SelfTimerEntityDeserializer()).create();
    }

    private static class SelfTimerEntityDeserializer implements JsonDeserializer<CustomizableEntity> {

        @Override
        public CustomizableEntity deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
            try {
