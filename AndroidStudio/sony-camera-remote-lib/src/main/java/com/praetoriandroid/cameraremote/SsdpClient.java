...
//    }
    ...
    private String processResponse(DatagramSocket socket) throws IOException, InvalidDataFormatException {
        socket.setSoTimeout(timeout);
        byte[] buffer = new byte[PACKET_BUFFER_SIZE];
        DatagramPacket answer = new DatagramPacket(buffer, buffer.length);
        socket.receive(answer);
        String ssdpReplyMessage = new String(answer.getData(), 0, answer.getLength());
...
//        String usn = findHeaderValue(ssdpReplyMessage, "USN"); // could be used as device id
    ...
    private String findHeaderValue(String ssdpMessage, String parameterName) throws InvalidDataFormatException {
        parameterName += ':';
