...
public class GetEventRequest extends BaseRequest<Boolean, GetEventResponse> {
    ...
    private Gson createGson() {
        return new GsonBuilder().registerTypeAdapter(EventEntity.class, new EventEntityDeserializer()).create();
    }

    private static class EventEntityDeserializer implements JsonDeserializer<EventEntity> {
        @Override
        public EventEntity deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
            try {
