...
public abstract class EventEntity {
    ...
    enum Type {
        availableApiList(AvailableApiList.class), cameraStatus(CameraStatus.class), zoomInformation(ZoomInformation.class), liveviewStatus(LiveViewStatus.class), // items 4-18 are skipped
        postviewImageSize(PostviewImageSize.class), selfTimer(SelfTimer.class), shootMode(ShootMode.class);

        private final Class<? extends EventEntity> entityClass;

        Type(Class<? extends EventEntity> entityClass) {
    ...
    public static class CameraStatus extends EventEntity {
        ...
        public enum Status {
            Error, NotReady, IDLE, StillCapturing, StillSaving, MovieWaitRecStart, MovieRecording, MovieWaitRecStop, MovieSaving, AudioWaitRecStart, AudioRecording, AudioWaitRecStop, AudioSaving
        }
    }
    ...
    public static class ZoomInformation extends EventEntity {
        ...
        @Override
        public String toString() {
            return getType() + ": position=" + zoomPosition + ", numberBox=" + zoomNumberBox + ", indexCurrentBox=" + zoomIndexCurrentBox + ", positionCurrentBox=" + zoomPositionCurrentBox;
        }
