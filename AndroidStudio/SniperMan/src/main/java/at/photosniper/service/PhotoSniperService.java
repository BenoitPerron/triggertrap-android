...
public class PhotoSniperService extends Service implements OutputListener, MicVolumeMonitor.VolumeListener, PhotoSniperLocationService.LocationListener, LightMonitor.LightListener {
    ...
    public void stopCurrentAction() {
        switch (mOnGoingAction) {
            ...
            case PhotoSniperApp.OnGoingAction.BANG:
                ...
                break;
            case PhotoSniperApp.OnGoingAction.BANG2:
                disableLightThreshold();
                break;
...
//                    mNotificationBuilder.getNotification());
    ...
    public void onLightUpdate(int amplitude) {
        if (mListener != null && !mIsRunningInForeground) {
            mListener.onLightUpdate(amplitude);
        }
...
//                    mNotificationBuilder.getNotification());
    ...
    public void onExceedLightThreshold(int amplitude) {
        ...
        if (mListener != null && !mIsRunningInForeground) {
            mListener.onLightExceedThreshold(amplitude);
        }
    }

    /*
     * LightSensor (Bang2) Control methods
     */
    ...
    public void stopLightSensor() {
        // Only stop the mic monitor is we are not watching it
        if (mOnGoingAction != PhotoSniperApp.OnGoingAction.BANG2) {
            mLightMonitor.stop();
        }
    }

    public void enableLightThreshold() {
        if (checkInProgressState()) {
            return;
        }
        mOnGoingAction = PhotoSniperApp.OnGoingAction.BANG2;
        mLightMonitor.enabledThreshold();
        mState = State.IN_PROGRESS;
    }

    public void disableLightThreshold() {
        mLightMonitor.disableThreshold();
        mState = State.IDLE;
        mOnGoingAction = PhotoSniperApp.OnGoingAction.NONE;
    }

    public void setLightSensitivity(int sensitivity) {
        mLightMonitor.setLightSensitivity(sensitivity);

    }

    public void setLightThreshold(int threshold) {
        mLightMonitor.setThreshold(threshold);
    }
...
//    }
    ...
    public interface PhotoSniperServiceListener {
        ...
        // Listeners for LightSensor
        void onLightUpdate(int amplitude);

        void onLightExceedThreshold(int amplitude);
