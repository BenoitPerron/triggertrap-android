package at.photosniper.location;
...
import at.photosniper.fragments.dialog.AlertErrorDialog;
...
public class PhotoSniperLocationService implements GooglePlayServicesClient.ConnectionCallbacks, GooglePlayServicesClient.OnConnectionFailedListener, LocationListener {

    public final static int CONNECTION_FAILURE_RESOLUTION_REQUEST = 9000;
    // Update frequency in seconds
    private static final int UPDATE_INTERVAL_IN_SECONDS = 1;
    private static final String TAG = PhotoSniperLocationService.class.getSimpleName();
    ...
    public PhotoSniperLocationService(Activity parentActivity) {
...
//		errorCode = ConnectionResult.SERVICE_VERSION_UPDATE_REQUIRED;
        ...
        switch (errorCode) {
            ...
            case ConnectionResult.SERVICE_VERSION_UPDATE_REQUIRED:
                serviceErrorDialog = GooglePlayServicesUtil.getErrorDialog(errorCode, mParentActivty, CONNECTION_FAILURE_RESOLUTION_REQUEST);

                // If Google Play services can provide an error dialog
                if (serviceErrorDialog != null) {
                    // Create a new DialogFragment for the error dialog
                    ServiceErrorDialogFragment errorFragment = new ServiceErrorDialogFragment();
                    // Set the dialog in the DialogFragment
                    errorFragment.setDialog(serviceErrorDialog);
                    // Show the error dialog in the DialogFragment
                    errorFragment.show(mParentActivty.getFragmentManager(), "Location Updates");
                }
                break;
    ...
    public void onConnectionFailed(ConnectionResult connectionResult) {
        ...
        if (connectionResult.hasResolution()) {
            try {
                ...
                connectionResult.startResolutionForResult(mParentActivty, CONNECTION_FAILURE_RESOLUTION_REQUEST);
                /*
    ...
    public interface LocationListener {
        void onDistanceChanged(float distance, float speed);
    }
}
