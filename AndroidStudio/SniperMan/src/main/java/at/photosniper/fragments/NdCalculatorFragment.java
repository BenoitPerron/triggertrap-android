...
public class NdCalculatorFragment extends TriggertrapFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View rootView = inflater.inflate(R.layout.nd_calc_mode, container, false);

        mNdValueArray = new double[]{TWELFTH_OF_A_MS, SIXTEENTH_OF_A_MS, TWENTIETH_OF_A_MS, QUARTER_OF_A_MS, THIRTYONE_OF_A_MS, FOURTY_OF_A_MS, HALF_OF_A_MS, SIXTYTHREE_OF_A_MS, EIGHTY_OF_A_MS, ONE_MS, ONE_AND_QUART_OF_MS, ONE_FIFTY_SIX_MS, TWO_MS, TWO_AND_HALF_MS, THREE_THIRTEEN_MS, FOUR_MS, FIVE_MS, SIX_AND_QUART_MS, EIGHT_MS, TEN_MS, THIRTEEN_MS, SEVENTEEN_MS, TWENTY_MS, TWENTY_FIVE_MS, THIRTYTHREE_MS, FOURTY_MS, FIFTY_MS, SIXTYSEVEN_MS, SEVENTYSEVEN_MS, HUNDRED_MS, EIGTH_OF_A_SEC, HUNDRED_SIXYSEVEN_MS, TWO_HUNDRED_MS, QUART_OF_A_SEC, THREE_HUNDRED_MS, FOUR_HUNDRED_MS, HALF_OF_A_SEC, SIX_HUNDRED_MS, EIGHT_HUNDRED_MS, ONE_SEC, THIRTEEN_HUNDRED_MS, SIXTEEN_HUNDRED_MS, TWO_SEC, TWENTYFIVE_HUNDRED_MS, THIRTYTWO_HUNDRED_MS, FOUR_SEC, FIVE_SEC, SIX_SEC, EIGHT_SEC, TEN_SEC, THIRTEEN_SEC, FIFTEEN_SEC, TWENTY_SEC, TWENTYFIVE_SEC, THIRTY_SEC};

        mNdFilterTextView = (TextView) rootView.findViewById(R.id.ndTFilterTypeText);
        ...
        mBaseShutterSpeedVals = getResources().getStringArray(R.array.base_shutter_speeds);
        mNdNameVals = getResources().getStringArray(R.array.nd_names);

        mNdFilterTextView.setText(getResources().getQuantityString(R.plurals.numberOfStops, 1, 1) + " (" + mNdNameVals[0] + ") " + getResources().getString(R.string.nd_filter_end));

        ArrayWheelDoubleAdapter ndValueAdapter = new ArrayWheelDoubleAdapter(getActivity(), mNdValues, mNdNameVals);
        ...
        ndValueWheel.addScrollingListener(new OnWheelScrollListener() {
            ...
            public void onScrollingFinished(AbstractWheel wheel) {
                ...
                mNdFilterTextView.setText(getResources().getQuantityString(R.plurals.numberOfStops, wheel.getCurrentItem() + 1, wheel.getCurrentItem() + 1) + " (" + mNdNameVals[wheel.getCurrentItem()] + ") " + getResources().getString(R.string.nd_filter_end));
    ...
    private String getNdTime(int stops, int shutterSpeed) {
        ...
        long result = (long) (mNdValueArray[shutterSpeed] * (Math.pow(2, stops + 1)));
        ...
        if (result > 360000000) {
            ...
            output = String.format(Locale.getDefault(), "%dh %02dm %02ds", TimeUnit.MILLISECONDS.toHours(result), TimeUnit.MILLISECONDS.toMinutes(result) - TimeUnit.HOURS.toMinutes(TimeUnit.MILLISECONDS.toHours(result)), TimeUnit.MILLISECONDS.toSeconds(result) - TimeUnit.MINUTES.toSeconds(TimeUnit.MILLISECONDS.toMinutes(result)));
